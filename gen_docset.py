...
docbases = ['gtkmm/stable', 'glibmm/stable']
...
for docbase in docbases:
    for file in os.listdir(os.path.join(docpath, docbase)):
        ...
        # Regex for title
        title_re = re.compile('(gtkmm|glibmm): *')

        # Regexes for file prefixes
        class_re = re.compile('(class|struct).*\.html')
        category_re = re.compile('group.*\.html')
        namespace_re = re.compile('namespace.*\.html')

        # Regexes for headers
        class_remove = re.compile('(gtkmm:|glibmm:|(Class|Struct) *\w* Reference)')
        category_remove = re.compile('(gtkmm|glibmm):')
